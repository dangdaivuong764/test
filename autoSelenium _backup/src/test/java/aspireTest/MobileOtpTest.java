...
public class MobileOtpTest {
	...
	/*
	 * Test case check valid OTP code
	 * Expected: the message successfully displayed
	 * */
	@Test
	public void testVerifyValidOTP() throws Exception {
		RegistrationPages register = new RegistrationPages(driver);
		MobileOtpPages mobileOtp= validRegisterStage(register);
		...
		waitAfterNavigateTo(3000);
		bussinessRole.checkTextnavigateToBussnessRole();
	}

	/*
	 * Test case check valid OTP code
	 * Expected: the message error displayed
	 * */
	@Test
	public void testVerifyInValidOTP() throws Exception {
		RegistrationPages register = new RegistrationPages(driver);
		MobileOtpPages mobileOtp= validRegisterStage(register);
		mobileOtp.checkTextPhoneOtp();
		waitAfterNavigateTo(500);
		mobileOtp.inputDigitsOTP(register.getDataTest("zero_otp"));
		mobileOtp.inputDigitsOTP(register.getDataTest("second_otp"));
		mobileOtp.inputDigitsOTP(register.getDataTest("third_otp"));
		mobileOtp.inputDigitsOTP(register.getDataTest("four_otp"));
		mobileOtp.checkMessageInvalidOTP();
	}

	private MobileOtpPages validRegisterStage(RegistrationPages register) throws Exception {
